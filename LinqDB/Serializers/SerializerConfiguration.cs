...
//}
public sealed class CustomSerializerUtf8Json{
    private readonly DispatchJsonFormatterResolver DispatchResolver=new();
    private readonly IJsonFormatterResolver Resolver;
    public CustomSerializerUtf8Json(){
        this.Resolver=Utf8Json.Resolvers.CompositeResolver.Create(
    ...
    public byte[]Serialize<T>(T value){
        this.DispatchResolver.Clear();
        return JsonSerializer.Serialize(value,this.Resolver);
    }
    public void Serialize<T>(Stream stream,T value){
        this.DispatchResolver.Clear();
        JsonSerializer.Serialize(stream,value,this.Resolver);
    }
    public T Deserialize<T>(byte[] bytes){
        this.DispatchResolver.Clear();
        return JsonSerializer.Deserialize<T>(bytes,this.Resolver);
    }
    public T Deserialize<T>(Stream stream){
        this.DispatchResolver.Clear();
        return JsonSerializer.Deserialize<T>(stream,this.Resolver);
    }
}
public sealed class CustomSerializerMessagePack{
    private readonly DispatchMessagePackFormatterResolver DispatchResolver=new();
    private readonly MessagePackSerializerOptions Options;
    public CustomSerializerMessagePack(){
