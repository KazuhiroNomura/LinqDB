...
public class DisplayClass<T>:MemoryPackFormatter<T>{
    ...
    private delegate T? delegate_Deserialize(ref Reader reader);

    private readonly delegate_Deserialize Read;
    private DisplayClass(){
        var Types1=new Type[1];
        Types1[0]=typeof(Reader).MakeByRefType();
        ...
        Array.Sort(Fields,(a,b)=>string.CompareOrdinal(a.Name,b.Name));
        {
            ...
            var Deserialize=new DynamicMethod("Deserialize",typeof(T),Types1,typeof(DisplayClass<T>),true){InitLocals=false};
            var I1=Deserialize.GetILGenerator();
            //I1.Emit(OpCodes.Ldarg_1);//value=new c_DisplayClass()
            I1.Emit(OpCodes.Newobj,ctor);
            var value=I1.M_DeclareLocal_Stloc(typeof(T));
            ...
            if(Fields_Length>0){
                while(true){
                    ...
                    I1.Ldflda(Field);//value.field
                    I1.Call(Extension.MethodDeserialize.MakeGenericMethod(Types1));//Deserialize(ref reader,ref value.field)
                    index++;
            ...
            I1.Ldloc_S(value);
            I1.Ret();
            this.Read=(delegate_Deserialize)Deserialize.CreateDelegate(typeof(delegate_Deserialize));
        }
    }
        ...
        if(!this.DictionarySerialize.TryGetValue(typeof(TBufferWriter),out var Delegate)){
            ...
            Array.Sort(Fields,(a,b)=>string.CompareOrdinal(a.Name,b.Name));
            {
                ...
                foreach(var Field in Fields){
                    ...
                    I0.Ldarg_1();//value
                    I0.Ldind_Ref();//*value
                    I0.Ldflda(Field);//ref value.field
                    I0.Call(Extension.MethodSerialize.MakeGenericMethod(MethodTypes));
                }
                I0.Ret();
