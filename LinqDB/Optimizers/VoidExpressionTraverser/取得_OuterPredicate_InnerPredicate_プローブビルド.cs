...
internal sealed class 取得_OuterPredicate_InnerPredicate_プローブビルド{
    ...
    private sealed class 判定_指定Parameterが存在する:VoidExpressionTraverser_Quoteを処理しない{
        ...
        private IEnumerable<ParameterExpression>指定Parameters=default!;
        ...
        protected override void Parameter(ParameterExpression Parameter){
            Debug.Assert(this.指定Parameters is not null);
            if(this.指定Parameters.Contains(Parameter)) this.Parameterが存在する=true;
        }
    }
    ...
    private IList<ParameterExpression>? OuterParameters;
    private IList<ParameterExpression>? InnerParameters;
    //private ParameterExpression? 内側Parameter;
    private Expression? OuterPredicate;
    //条件式CNFに分解して取得しその式のラムダの深さをListに入れる
    //ソートしてつまりラムダの浅い順に入れる
    //そのリストを走査し
    public(Expression? OuterPredicate,Expression? InnerPredicate,List<(Expression プローブ,Expression ビルド)> Listプローブビルド) 実行(Expression predicate_Body,IList<ParameterExpression> OuterParameters,IList<ParameterExpression>InnerParameters){
    ...
    private Expression? 等号が出現した時にDictionaryHashとEqualに分離(Expression e,Expression Left,Expression Right){
        Debug.Assert(this.OuterParameters is not null);
        if(HashEqualを設定(Right,Left)||HashEqualを設定(Left,Right)) return null;
        Debug.Assert(this.InnerParameters is not null);
        if(this.判定_指定Parameter有_他Parameter無_Lambda内部走査.実行(e,this.InnerParameters!)){
        ...
        bool HashEqualを設定(Expression プローブ,Expression ビルド){
            ...
            if(this._判定_指定Parameterが存在する.実行(ビルド,this.InnerParameters)){
                if(this.判定_指定Parameter無_他Parameter有.実行(プローブ,this.InnerParameters)){
                    //_ListParameters[0]={}
                    //_ListParameters[1]={a}
                    //_ListParameters[2]={b}
                    //_ListParameters[3]={c}
    ...
    private Expression? PrivateTraverseNullable(Expression Expression0){
        ...
        Debug.Assert(this.InnerParameters is not null);
        if(this.判定_指定Parameter有_他Parameter無_Lambda内部走査.実行(Expression0,this.InnerParameters!)){
