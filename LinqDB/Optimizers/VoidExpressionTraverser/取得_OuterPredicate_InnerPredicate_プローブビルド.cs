...
internal sealed class 取得_OuterPredicate_InnerPredicate_プローブビルド:ReturnExpressionTraverser_Quoteを処理しない {
    private readonly 判定_指定Parameter無_他Parameter有 判定_指定Parameter無_他Parameter有;
    private sealed class 判定_Parametersが1つ以上存在する:VoidExpressionTraverser_Quoteを処理しない {
        ...
        protected override void Parameter(ParameterExpression Parameter) {
            ...
            if(this.指定Parameter==Parameter)
                this.Parameterが存在する=true;
        }
    }
    private readonly 判定_Parametersが1つ以上存在する _判定_Parametersが1つ以上存在する = new();
    ...
    public 取得_OuterPredicate_InnerPredicate_プローブビルド(作業配列 作業配列,判定_指定Parameter無_他Parameter有 判定_指定Parameter無_他Parameter有,判定_指定Parameter有_他Parameter無_Lambda内部走査 判定_指定Parameter有_他Parameter無_Lambda内部走査,ブローブビルドExpressionEqualityComparer ブローブビルドExpressionEqualityComparer) : base(作業配列) {
    ...
    private Expression? 等号が出現した時にDictionaryHashとEqualに分離(Expression e,Expression Left,Expression Right) {
        //var 判定_指定Parametersが存在しない=new 判定_指定Parametersが存在しない();
        ...
        bool HashEqualを設定(Expression プローブ,Expression ビルド) {
            ...
            Debug.Assert(this.内側Parameter is not null);
            if(
                this._判定_Parametersが1つ以上存在する.実行(ビルド,this.内側Parameter!)&&
                this.判定_指定Parameter無_他Parameter有.実行(プローブ,this.内側Parameter!)
            ) {
                //_ListParameters[0]={}
                //_ListParameters[1]={a}
                //_ListParameters[2]={b}
    ...
    private Expression? PrivateTraverseNullable(Expression Expression0) {
        switch(Expression0.NodeType) {
            case ExpressionType.OrElse:return Expression0;
