...
//,Generic.IEqualityComparer<SymbolDocumentInfo>
{
    ...
    protected virtual bool ProtectedAssign(BinaryExpression x_Assign,BinaryExpression y_Assign){
        ...
        if(!this.ProtectedEquals(x_Assign.Right,y_Assign.Right)) return @false;
        if(!this.InternalEquals(x_Assign.Conversion,y_Assign.Conversion)) return @false;
    ...
    internal bool InternalEquals(Expression? a0,Expression? b0){
        if(a0 is null) return b0 is null;
        if(b0 is null) return @false;
    ...
    protected bool ProtectedEquals(Expression a0,Expression b0){
        ...
        if(a1.NodeType!=b1?.NodeType||a1.Type!=b1.Type) return @false;
