    ...
    public override bool Equals(object? x,object? y){
        ...
        if(x_GetType.IsAnonymous()) {
            if(y_GetType.IsAnonymous()) {
                ...
                return true;
            } else if(y is ITuple yt) {
                var x_Properties = x_GetType.GetProperties();
                var x_Properties_Length = x_Properties.Length;
                if(x_Properties_Length!=yt.Length) return false;
                for(var Index = 0;Index<x_Properties_Length;Index++) {
                    var x0 = x_Properties[Index].GetMethod!.Invoke(x,Array.Empty<Object>())!;
                    var y0 = yt[Index];
                    if(!this.Equals(x0,y0)) return false;
        ...
        } else if(x is ITuple xt) {
            if(y_GetType.IsAnonymous()) {
                var y_Properties = y_GetType.GetProperties();
                var y_Properties_Length = y_Properties.Length;
                if(xt.Length!=y_Properties_Length) return false;
                for(var Index = 0;Index<y_Properties_Length;Index++) {
                    var x0 = xt[Index];
                    var y0 = y_Properties[Index].GetValue(y);
                    if(!this.Equals(x0,y0)) return false;
                }
                return true;
            } else if(y is ITuple yt) {
                for(var Index = 0;Index<xt.Length;Index++) {
                    var x0 = xt[Index];
                    var y0 = yt[Index];
                    if(!this.Equals(x0,y0)) return false;
                }
                ...
                var y_Properties = y_GetType.GetProperties();
                foreach(var y_Property in y_Properties) {
                    var x0 = x_GetType.GetProperty(y_Property.Name)!.GetValue(x);
                    var y0 = y_Property.GetValue(y);
                    if(!this.Equals(x0,y0)) return false;
                }
                return false;
            }
