...
//,Generic.IEqualityComparer<SymbolDocumentInfo>
{
    ...
    protected virtual bool ProtectedAssign(BinaryExpression x_Assign,BinaryExpression y_Assign){
        ...
        //var x_Index1 = スコープParameters.IndexOf(x_Parameter);
        //var y_Index1 = スコープParameters.IndexOf(y_Parameter);
        //if(x_Index1!=y_Index1) return @false;
        //if(x_Index1>=0) return true;
        //Let(x=>.v),Let(y=>.w)は一致しない。
        //Let(x=>.v=x),Let(y=>.w=y)は一致する。大域変数.v,.wが代入左辺地なら一致する。
    ...
    public abstract bool Equals(ParameterExpression? x,ParameterExpression? y);
...
//public class ExpressionEqualityComparer:AExpressionEqualityComparer,Generic.IEqualityComparer<LabelTarget>,Generic.IEqualityComparer<CatchBlock>,Generic.IEqualityComparer<CSharpArgumentInfo>,Generic.IEqualityComparer<SwitchCase>,Generic.IEqualityComparer<MemberBinding>,Generic.IEqualityComparer<MemberAssignment>,Generic.IEqualityComparer<MemberListBinding>,Generic.IEqualityComparer<MemberMemberBinding>,Generic.IEqualityComparer<ElementInit>,Generic.IEqualityComparer<SymbolDocumentInfo>{
//    private static bool @false=>false;
//    private 汎用Comparer ObjectComparer=>new();
//    ///// <summary>
//    ///// 比較するときに可視パラメーター
//    ///// </summary>
...
//    private readonly Generic.List<ParameterExpression> x_Parameters = new();
//    private readonly Generic.List<ParameterExpression> y_Parameters = new();
//    private readonly Generic.List<LabelTarget> x_LabelTargets = new();
//    private readonly Generic.List<LabelTarget> y_LabelTargets = new();
//    private void Clear(){
...
//                return this.T((BinaryExpression)a1,(BinaryExpression)b1);
//            case ExpressionType.Assign: {
//                var x_Assign = (BinaryExpression)a1;
//                var y_Assign = (BinaryExpression)b1;
//                var x_Left = x_Assign.Left;
//                var y_Left = y_Assign.Left;
//                if(x_Left.NodeType!=y_Left.NodeType) return @false;
//                if(x_Left.NodeType!=ExpressionType.Parameter) return this.T(x_Assign,y_Assign);
//                if(!this.PrivateEquals(x_Assign.Right,y_Assign.Right))
//                    return @false;
//                if(!this.InternalEquals(x_Assign.Conversion,y_Assign.Conversion))
//                    return @false;
//                var x_Parameter= (ParameterExpression)x_Left;
...
//                if(x_ラムダ跨ぎParameters.Contains(x_Parameter))
//                    if(y_ラムダ跨ぎParameters.Contains(y_Parameter))
//                        return true;
//                    else
//                        return @false;
//                else
...
//        }
//    }
//    protected bool T(BinaryExpression x,BinaryExpression y) => x.Method==y.Method&&this.PrivateEquals(x.Left,y.Left)&&this.PrivateEquals(x.Right,y.Right)&&this.InternalEquals(x.Conversion,y.Conversion);
//    protected bool T(BlockExpression x,BlockExpression y){
//        //if(!this.T(x.Variables,y.Variables)) return @false;
...
//    }
//    protected bool T(ConditionalExpression x,ConditionalExpression y) =>
//        this.PrivateEquals(x.Test,y.Test)&&
//        this.PrivateEquals(x.IfTrue,y.IfTrue)&&
//        this.PrivateEquals(x.IfFalse,y.IfFalse);
//    protected bool T(ConstantExpression x,ConstantExpression y)=>
//        ReferenceEquals(x.Value,y.Value)||
//        x.Type==y.Type&&this.ObjectComparer.Equals(x.Value,y.Value);
//        //Equals(x.Value,y.Value);
...
//    //}
//    protected bool T(MemberInitExpression x,MemberInitExpression y) =>
//        this.PrivateEquals(x.NewExpression,y.NewExpression)&&
//        x.Bindings.SequenceEqual(y.Bindings);
//    protected bool T(NewArrayExpression x,NewArrayExpression y) => this.SequenceEqual(x.Expressions,y.Expressions);
//    //パラメータが同じならオーバーロードでコンストラクタが異なることはあり得ない。
//    //しかし異なる型の同じパラメータコンストラクタはあり得る。
//    protected bool T(NewExpression x,NewExpression y) =>
...
//        var r = this.PrivateEquals(x.Body,y.Body);
//        Debug.Assert(this.x_LabelTargets.Count==this.y_LabelTargets.Count);
...
//    protected bool T(ListInitExpression x,ListInitExpression y) =>
//        this.PrivateEquals(x.NewExpression,y.NewExpression)&&
//        x.Initializers.SequenceEqual(y.Initializers);
//    protected bool T(LoopExpression x,LoopExpression y) {
//        var x_BreakLabel = x.BreakLabel;
//        var y_BreakLabel = y.BreakLabel;
//        var x_ContinueLabel = x.ContinueLabel;
...
//    protected bool T(IndexExpression x,IndexExpression y) =>
//        x.Indexer==y.Indexer&&
//        this.PrivateEquals(x.Object,y.Object)&&
//        this.SequenceEqual(x.Arguments,y.Arguments);
//    protected bool T(MethodCallExpression x,MethodCallExpression y){
//        var x_Method = x.Method;
//        var y_Method = y.Method;
...
//    }
//    protected bool T(InvocationExpression x,InvocationExpression y) =>
//        this.PrivateEquals(x.Expression,y.Expression)&&
//        this.SequenceEqual(x.Arguments,y.Arguments);
//    protected bool T(GotoExpression x,GotoExpression y) =>
//        this.x_LabelTargets.IndexOf(x.Target)==this.y_LabelTargets.IndexOf(y.Target)&&
//        this.InternalEquals(x.Value,y.Value);
...
//        return true;
//    }
//    protected bool T(ParameterExpression x,ParameterExpression y) {
...
//        //探索開始時点の
//        //var スコープParameters = this.スコープParameters;
//        //var x_Index1 = スコープParameters.IndexOf(x);
//        //var y_Index1 = スコープParameters.IndexOf(y);
//        //if(x_Index1!=y_Index1) return @false;
//        //if(x_Index1>=0) return true;
//        //Let(x=>.v),Let(y=>.w)は一致しない。
//        //Let(x=>.v=x),Let(y=>.w=y)は一致する。大域変数.v,.wが代入左辺地なら一致する。
...
//        var x_ラムダ跨ぎParameters=this.x_ラムダ跨ぎParameters;
//        var y_ラムダ跨ぎParameters=this.y_ラムダ跨ぎParameters;
//        if(x_ラムダ跨ぎParameters.Contains(x))return y_ラムダ跨ぎParameters.Contains(y);
//        if(y_ラムダ跨ぎParameters.Contains(y))return @false;
//        x_ラムダ跨ぎParameters.Add(x);
//        y_ラムダ跨ぎParameters.Add(y);
//        return true;
//#else
//        //lambda用comparerはここでfalse
//        //    Comparerフォルダに
...
//        var x_ラムダ跨ぎParameters = this.x_ラムダ跨ぎParameters;
//        var y_ラムダ跨ぎParameters = this.y_ラムダ跨ぎParameters;
//        var x_index = x_ラムダ跨ぎParameters.IndexOf(x);
//        var y_index = y_ラムダ跨ぎParameters.IndexOf(y);
...
//#endif
//    }
//    protected bool T(RuntimeVariablesExpression x,RuntimeVariablesExpression y) => x.Variables.SequenceEqual(y.Variables);
//    protected bool T(SwitchExpression x,SwitchExpression y) {
//        if(x.Comparison!=y.Comparison)
//            return @false;
//        if(!this.PrivateEquals(x.DefaultBody,y.DefaultBody))
//            return @false;
//        if(!this.PrivateEquals(x.SwitchValue,y.SwitchValue))
//            return @false;
//        var x_Cases = x.Cases;
//        var y_Cases = y.Cases;
//        var x_Cases_Count = x_Cases.Count;
...
//            var x_Case = x_Cases[c];
//            var y_Case = y_Cases[c];
//            if(!this.PrivateEquals(x_Case.Body,y_Case.Body))
//                return @false;
//            if(!this.SequenceEqual(x_Case.TestValues,y_Case.TestValues))
//                return @false;
//        }
...
//    }
//    protected bool T(TryExpression x,TryExpression y) {
//        if(!this.PrivateEquals(x.Body,y.Body))return @false;
//        if(!this.InternalEquals(x.Fault,y.Fault))return @false;
//        if(!this.InternalEquals(x.Finally,y.Finally))return @false;
//        var x_Handlers = x.Handlers;
//        var y_Handlers = y.Handlers;
...
//    protected bool T(UnaryExpression x,UnaryExpression y) =>
//        x.Method==y.Method&&x.Type==y.Type&&(
//            x.Operand is not null&&y.Operand is not null&&this.PrivateEquals(x.Operand,y.Operand)||
//            x.Operand is null&&y.Operand is null);
//    public int GetHashCode(ElementInit obj)=>0;
//    public bool Equals(ElementInit? x,ElementInit? y) {
//        this.Clear();
...
//        if(x==null&&y!=null||x!=null&&y==null) return @false;
//        if(x!.AddMethod!=y!.AddMethod) return @false;
//        return x.Arguments.SequenceEqual(y.Arguments);
//    }
//    public int GetHashCode(MemberBinding obj)=>0;
//    public bool Equals(MemberBinding? x,MemberBinding? y) {
//        this.Clear();
...
//        if(x==null&&y!=null||x!=null&&y==null) return @false;
//        if(x!.Member!=y!.Member) return @false;
//        return x.Initializers.SequenceEqual(y.Initializers);
//    }
//    public int GetHashCode(MemberMemberBinding obj)=>0;
...
//        if(x==null&&y!=null||x!=null&&y==null) return @false;
//        if(x!.Member!=y!.Member) return @false;
//        return x.Bindings.SequenceEqual(y.Bindings);
//    }
//    public int GetHashCode(CSharpArgumentInfo obj)=>0;
...
//        if(x==y) return true;
//        if(x==null&&y!=null||x!=null&&y==null) return @false;
//        return this.PrivateEquals(x!.Body,y!.Body)&&x.TestValues.SequenceEqual(y.TestValues);
//    }
//}
