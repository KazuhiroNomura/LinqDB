...
/// </summary>
internal sealed partial class 変換_Tryの先行評価:ReturnExpressionTraverser{
    ...
    private sealed class 変換Try:ReturnExpressionTraverser{
                ...
                switch(Expression0.NodeType){
                    case ExpressionType.Try:{
                        ...
                        for(var a=0;a<Try0_Handlers_Count;a++){
                            ...
                            //ListAssign.Add(Try1_Handler_Body);
        ...
        //internal readonly List<Expression> ListExpression;
        ...
        protected override Expression Traverse(Expression Expression0){
            ...
            switch(Expression0.NodeType){
                case ExpressionType.Try:{
                    ...
                    //    //var 全てにAssignがある=true;
                    //    var 全てにAssignがあってLeftが同一Parameter = true;
                    //    Expression? Parameter = null;
                    //    if(Try0_Body.NodeType==ExpressionType.Assign)
                    //        Parameter=((BinaryExpression)Try0_Body).Left;
                    //    else
                    //        全てにAssignがあってLeftが同一Parameter=false;
                    //    for(var a = 0;a<Try0_Handlers_Count;a++) {
                    //        var Try0_Handler = Try0_Handlers[a];
                    //        var Try0_Handler_Body = Try0_Handler.Body;
                    //        if(Try0_Handler_Body.NodeType==ExpressionType.Assign) {
                    //            if(Parameter!=((BinaryExpression)Try0_Handler_Body).Left) 全てにAssignがあってLeftが同一Parameter=false;
                    //        } else
                    //            全てにAssignがあってLeftが同一Parameter=false;
                    //    }
                    //    if(全てにAssignがあってLeftが同一Parameter)
                    //        return Try0;
                    //}
                    {
                        Debug.Assert(!(Try0.Finally is not null&&Try0.Fault is not null));
                        ...
                        for(var a=0;a<Try0_Handlers_Count;a++){
                            ...
                            //var Try1_Handler_Body=Expression.Assign(
                            //    新Parameter,
                            //    Try0_Handler.Body
                            //);
                            //ListAssign.Add(Try1_Handler_Body);
                        ...
                        //var Try1_Finally=this.TraverseNullable(Try0.Finally);
                        TryExpression Try1;
                        if(Try0.Fault is not null){
                            Debug.Assert(Try0.Finally is null);
                            var Try1_Fault=this.Traverse(Try0.Fault);
        ...
        //    var Block1_Expressions=new Expression[Block0_Expressions_Count];
        //    for(var a=0;a<Block0_Expressions_Count;a++)
        //        Block1_Expressions[a]=this.PrivateTry(Block0_Expressions[a]);
        //    return Expression.Block(Block0.Variables,Block1_Expressions);
        //}
        //private Expression PrivateTry(Expression Expression0){
        //    if(Expression0.NodeType==ExpressionType.Try) return this.Try((TryExpression)Expression0);
        //    return this.ListAssign[this.ListIndex++];
        //}
    }
    ...
    //private List<ParameterExpression> ListParameter{
    //    get=>this._変換Try.ListParameter;
    //    set=>this._変換Try.ListParameter=value;
    //}
