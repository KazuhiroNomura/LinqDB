...
public sealed class SetGroupingAscList<TKey,TElement>:ImmutableSet<GroupingAscList<TKey,TElement>>{
...
#pragma warning disable CA1823 // 使用されていないプライベート フィールドを使用しません
    //private new static readonly Serializers.MemoryPack.Formatters.Sets.SetGroupingAscList<TKey,TElement> InstanceMemoryPack=Serializers.MemoryPack.Formatters.Sets.SetGroupingSet<TKey,TElement>.Instance;
    //private new static readonly Serializers.MessagePack.Formatters.Sets.SetGroupingAscList<TKey,TElement> InstanceMessagePack=Serializers.MessagePack.Formatters.Sets.SetGroupingSet<TKey,TElement>.Instance;
    //private new static readonly Serializers.Utf8Json.Formatters.Sets.SetGroupingAscList<TKey,TElement> InstanceUtf8Json=Serializers.Utf8Json.Formatters.Sets.SetGroupingSet<TKey,TElement>.Instance;
...
//#pragma warning restore CA1823 // 使用されていないプライベート フィールドを使用しません
    ...
    /// 既定コンストラクタ
    /// </summary>
    public SetGroupingAscList()=>this.KeyComparer=Generic.EqualityComparer<TKey>.Default;
    /// <summary>
    /// 比較方法を指定したコンストラクタ
    /// </summary>
    /// <param name="KeyComparer">比較方法</param>
    public SetGroupingAscList(Generic.IEqualityComparer<TKey> KeyComparer)=>this.KeyComparer=KeyComparer;
    ...
    internal void AddKeyValue(TKey Key,TElement Value) {
        ...
        if(this.InternalAdd前半(out var 下限,out var 上限,out var TreeNode,HashCode)) {
            ...
            while(true) {
                ...
                if(LinkedNode_LinkedNodeItem is null) {
                    LinkedNode._LinkedNodeItem=new LinkedNodeItemT(new GroupingAscList<TKey,TElement>(Key,Value));
                    this._LongCount++;
                    return;
                }
        ...
        InternalAdd後半(
            ...
            TreeNode,
            HashCode,
            new LinkedNodeItemT(new GroupingAscList<TKey,TElement>(Key,Value))
        );
        this._LongCount++;
    }
