...
public class 変換_OuterPredicate_InnerPredicate_プローブビルド:共通{
    ...
    [Fact]
    public void PrivateTraverseNullable(){
        var s=CreateSet();
        //switch(Expression0.NodeType) {
        //    case ExpressionType.OrElse:
        this.Expression実行AssertEqual(()=>(1).Let(p=>CreateSet().Where(q=>p==q||q==p)));
        //    case ExpressionType.AndAlso: {
        //        if(Binary1_Left is null) return Binary1_Right;
        this.Expression実行AssertEqual(()=>(1).Let(p=>CreateSet().Where(q=>p==q&&q==p)));
        //        if(Binary1_Right is null) return Binary1_Left;
        this.Expression実行AssertEqual(()=>(1).Let(p=>CreateSet().Where(q=>p%2==0&&p==q)));
        this.Expression実行AssertEqual(()=>(1).Let(p=>CreateSet().Where(q=>p%2==0&&p>0)));
        //    }
        //    case ExpressionType.Equal: {
        //        if(Binary_Left.Type.IsPrimitive) return this.等号が出現した時にDictionaryHashとEqualに分離(Binary0,Binary_Left,Binary0.Right);
        this.Expression実行AssertEqual(()=>(1).Let(p=>CreateSet().Where(q=>p==q)));
        this.Expression実行AssertEqual(()=>(1m).Let(p=>CreateSet().Where(q=>p==0)));
        //    }
        //    case ExpressionType.Call: {
        //        if(nameof(Equals)==MethodCall.Method.Name) {
        //            if(Reflection.Object.Equals_==MethodCall.Method) {
        this.Expression実行AssertEqual(()=>((object)1).Let(p=>CreateSet().Where(q=>p.Equals(q))));
        //            }
        ...
        this.Expression実行AssertEqual(()=>(1).Let(p=>CreateSet().Where(q=>p.Equals(q))));
        this.Expression実行AssertEqual(()=>new Class().Let(p=>CreateSet().Where(q=>q.Equals(p))));
        //        }
        //    }
        //}
    ...
    public void Call_GroupBy1(){
        ...
        this.Expression実行AssertEqual(()=>new int[10].GroupBy(p=>p,p=>p+p,(key,g)=>key+g.Count(),EqualityComparer<decimal>.Default));
        this.Expression実行AssertEqual(()=>new Set<int>().GroupBy(p=>new{p},p=>p+p,(key,g)=>key.p+g.Count()));
    }
    [Fact]
    public void Call_GroupBy2(){
        this.Expression実行AssertEqual(()=>new int[10].GroupBy(p=>new{p}));
        this.Expression実行AssertEqual(()=>new int[10].GroupBy(p=>p,EqualityComparer<decimal>.Default));
        this.Expression実行AssertEqual(()=>new Set<int>().GroupBy(p=>new{p}));
        this.Expression実行AssertEqual(()=>new Set<int>().GroupBy(p=>new{p},p=>p*p));
    }
    [Fact]
    public void Call_GroupBy_GroupBy_keySelector_resultSelector(){
    ...
    [Fact]
    public void Call_GroupBy_GroupBy_keySelector_elementSelector_resultSelector(){
    ...
    [Fact]
    public void Call_GroupBy_GroupBy_keySelector(){
    ...
    [Fact]
    public void Call_UnionBy_Anonymous(){
        var a=new[]{3,5,7};
        var b=new[]{4,6,8};
    ...
    //}
    [Fact]
    public void 共通AnonymousValueTuple(){
        //if(MethodCall1_Object is NewExpression LNew&&MethodCall1_Arguments_0 is NewExpression RNew) {
        //    for(var a = 1;a<LNew_Arguments_Count;a++) {
        this.Expression実行AssertEqual(()=>new{a="a",b="b"}.Equals(new{a="a",b="b"}));
        this.Expression実行AssertEqual(()=>new ValueTuple<int,int>(1,2).Equals(new ValueTuple<int,int>(1,2)));
    ...
    [Fact]
    public void 条件が合えば内部SelectManyのselector_Bodyに外部メソッドを入れる0(){
        //if(ループ展開可能メソッドか(MethodCall1_Arguments_0,out var MethodCall1_MethodCall)) {
    ...
    [Fact]
    public void 条件が合えば内部SelectManyのselector_Bodyに外部メソッドを入れる1(){
        //if(ループ展開可能メソッドか(MethodCall1_Arguments_0,out var MethodCall1_MethodCall)) {
        //    switch(MethodCall1_MethodCall.Method.Name) {
        //        case nameof(ExtensionSet.SelectMany): {
        //            if(Reflection.ExtensionEnumerable.SelectMany_indexSelector!=MethodCall1_MethodCall.Method.GetGenericMethodDefinition()) {
    ...
    [Fact]
    public void 条件が合えば内部SelectManyのselector_Bodyに外部メソッドを入れる2(){
        //if(ループ展開可能メソッドか(MethodCall1_Arguments_0,out var MethodCall1_MethodCall)) {
        //    switch(MethodCall1_MethodCall.Method.Name) {
        //        case nameof(ExtensionSet.SelectMany): {
        //            if(Reflection.ExtensionEnumerable.SelectMany_indexSelector!=MethodCall1_MethodCall.Method.GetGenericMethodDefinition()) {
    ...
    [Fact]
    public void 内部SelectManyのselector_Bodyに外部メソッドを入れる0(){
        this.Expression実行AssertEqual(()=>CreateSet().SelectMany(p=>CreateSet()).OfType<string>());
        this.Expression実行AssertEqual(()=>CreateEnum().SelectMany((p,index)=>CreateEnum()).OfType<object>());
    }
    [Fact]
    public void 内部SelectManyのselector_Bodyに外部メソッドを入れる1(){
    ...
    [Fact]
    public void 内部SelectManyのselector_Bodyに外部メソッドを入れる2(){
        //if(MethodCall1_MethodCall_Arguments[1] is LambdaExpression selector0) {
    ...
    [Fact]
    public void 共通後処理内部SelectManyのselectorBodyに外部メソッドを入れる(){
    ...
    public void Select_Where再帰で匿名型を走査(){
        ...
        this.Expression実行AssertEqual(()=>CreateSet()
            .Select(p=>new ValueTuple<int,int,int,int,int,int,int,ValueTuple<int,int,int,int,int,int,int,ValueTuple<int>>>(p,p,p,p,p,p,p,new ValueTuple<int,int,int,int,int,int,int,ValueTuple<int>>(p,p,p,p,p,p,p,new ValueTuple<int>(p)))).Where(
                p=>
                    p.Item1==0&&
                    p.Item2==0&&
                    p.Item3==0&&
    ...
    [Fact]
    public void Call_取得_Parameter_OuterPredicate_InnerPredicate(){
    ...
    [Fact]
    public void KeySelectorの匿名型をValueTuple(){
        var s=new Set<int>();
        ...
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,o=>new{key=o},i=>new{key=i},(o,i)=>new{o,i}));
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,o=>new{key0=o,key1=o},i=>new{key0=i,key1=i},(o,i)=>new{o,i}));
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,o=>new{key=o},i=>F(new{key=i}),(o,i)=>new{o,i}));
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,o=>F(new{key=o}),i=>new{key=i},(o,i)=>new{o,i}));
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,o=>F(new{key=o}),i=>F(new{key=i}),(o,i)=>new{o,i}));
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,o=>new{key=o},Anonymous((int i)=>new{key=i}),(o,i)=>new{o,i}));
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,Anonymous((int o)=>new{key=o}),i=>new{key=i},(o,i)=>new{o,i}));
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,Anonymous((int o)=>new{key=o}),Anonymous((int i)=>new{key=i}),(o,i)=>new{o,i}));
        this.Expression実行AssertEqual(()=>s.GroupJoin(s,o=>o,i=>i,(o,i)=>new{o,i}));
