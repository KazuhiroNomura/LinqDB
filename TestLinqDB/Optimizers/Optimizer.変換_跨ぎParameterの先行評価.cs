...
public class 変換_跨ぎParameterの先行評価:共通{
    ...
    [Fact]public void 取得_先行評価式_Traverse(){
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => s.Join(s,o => new { key = o },i => new { key = i },(o,i) => o+i));
        //switch(e.NodeType) {
        //    case ExpressionType.Constant: {
        //        if(ILで直接埋め込めるか((ConstantExpression)e))return;
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>0);
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s);
        //    }
        //    case ExpressionType.Default:return;
        this.MemoryMessageJson_Expression_コンパイル実行(Expression.Lambda<Action>(Expression.Default(typeof(int))));
        //    case ExpressionType.Parameter: {
        //        if(this.ラムダ跨ぎParameters.Contains(e)||this.ループ跨ぎParameters.Contains(e))return;
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>"".Let(b=>a)));
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>"".Inline(b=>a)));
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>a));
        //    }
        //}
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>"".Inline(b=>a)));
        //            }
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>"".Inline(b=>b)));
        //            if(Result==判定_移動できるか.EResult.NoLoopUnrollingがあったので移動できない)return;
        //this.共通MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>"".Inline(b=>a).NoLoopUnrolling()));
        //        }
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>"".Let(b=>a)));
        //            }
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>"".Let(b=>b)));
        //            if(Result==判定_移動できるか.EResult.NoLoopUnrollingがあったので移動できない)return;
        //this.共通MemoryMessageJson_Expression_コンパイルリモート実行(() => "".Let(a=>"".Let(b=>a).NoLoopUnrolling()));
        //        }
        //    }
        //}
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => s.ToString());
    }
    [Fact]public void 取得_先行評価式_Call(){
        var s = new Set<int>();
        //if(Reflection.Helpers.NoLoopUnrolling==MethodCall_GenericMethodDefinition)
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>ExtensionSet.Inline(()=>"").NoLoopUnrolling());
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>ExtensionSet.Inline(()=>""));
        //                }else{
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>ExtensionSet.Inline((Func<decimal>)(()=>1m)));
        //                }
        //            }else{
        //                if(MethodCall0_Arguments_1 is LambdaExpression Lambda0) {
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Inline(s=>s+"x"));
        //                } else {
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Inline((Func<string,string>)(s=>s+"x")));
        //                }
        //            }
        //        }
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Let(b=>s.Select(a=>a+a)).Except(s.Select(a=>a+a)));
        //            if(this.結果Expression is not null)
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Select(a=>a+a).Except(s.Select(a=>a+a)));
        //            if(MethodCall.Arguments.Count==3)
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Except(s,EqualityComparer<int>.Default));
        //            break;
        //        }
        //        default: {
        //            if(this.結果Expression is not null)
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Let(b=>s.Select(a=>a+a)).Select(a=>a+a));
        //            for(var a = 1;a<MethodCall0_Arguments_Count;a++)
        //                if(巻き上げ処理(MethodCall0_Arguments[a])) 
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Select(a=>1m));
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Select(a=>a+1));
        //        }
        //    }
        //} else {
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Let(s=>""));
        //}
        //bool 巻き上げ処理(Expression Expression0) {
        //    if(Expression0 is LambdaExpression Lambda0) {
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Select(a=>1m));
        //    }
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Select((Func<int,decimal>)(a=>1m)));
        //}
    }
    ...
    [Fact]public void 変換_先行評価式_Call(){
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Select(a=>a+a));
        //switch(MethodCall_GenericMethodDefinition.Name) {
        //    case nameof(ExtensionSet.Inline): {
        //        if(MethodCall0_Arguments.Count==1) {
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Let(a=>ExtensionSet.Inline(()=>1m)));
        //        }else{
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Let(a=>"".Inline(a=>1m)));
        //this.共通MemoryMessageJson_Expression_コンパイルリモート実行(()=>ExtensionSet.Inline(()=>"".Inline(s=>s+"x")));
        //        }
        //    }
        //    default: {
        //        for(var a=1;a<MethodCall0_Arguments_Count;a++){
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Select(a=>s));
        //        }
        //    }
        //}
    }
    ...
    [Fact]public void Call(){
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => s.Join(s,o => o,i => i,(o,i) =>o+i));
        //if(Reflection.ExtensionSet.Inline1==GetGenericMethodDefinition(MethodCall0.Method)){
        //    if(MethodCall0_Arguments_0 is LambdaExpression Lambda0)
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>LinqDB.Sets.ExtensionSet.Inline(()=>""));
        //    else
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>LinqDB.Sets.ExtensionSet.Inline((Func<string>)(()=>"")));
        //}else if(Reflection.ExtensionSet.Inline2==GetGenericMethodDefinition(MethodCall0.Method)) {
        //    if(MethodCall0_Arguments_1 is LambdaExpression Lambda0)
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Inline(s=>""));
        //    else
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"".Inline((Func<string,string>)(s=>"")));
        //}
        //for(var a = 1;a<MethodCall0_Arguments_Count;a++)
        //    if(MethodCall0_Argument is LambdaExpression Lambda0)
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.Select(p=>p+1));
        //    else
    ...
    [Fact]public void 外だし(){
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"a".Let(o=>o.Let(i=>o)));
        //        } else{
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>"a".Inline(a=>a.Inline(b=>a.Inline(c=>a))));
        //        }
        //        do {
        this.MemoryMessageJson_Expression_コンパイルリモート実行(() => s.Join(s,o => o,i => i,(o,i) => new { o,i }));
        ...
        this.MemoryMessageJson_Expression_コンパイルリモート実行(()=>s.SelectMany(o=>s.Let(i=>new{o,i}).i.Where(i=>i==3)));
    }
