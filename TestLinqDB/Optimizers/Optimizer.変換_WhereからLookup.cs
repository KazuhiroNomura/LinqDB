...
public class 変換_WhereからLookup:共通{
    ...
    [Fact]public void xxx(){
        ...
        this.コンパイル実行(()=>st.SelectMany(o=>st.Where(i=>i.Key.Equals(o.Key))));
    }
    class SetT:Set<OI>{
    ...
    [Fact]public void Call(){
        ...
        //            }
        this.コンパイル実行(() => s.Join(s,o => o,i => i,(o,i) => new { o,i }).Where(p => p.o==0));
        ...
        this.コンパイル実行(()=>st.SelectMany(o=>st.Where(i=>i.Key.Equals(o.Key))));
        //                            } else {
        this.コンパイル実行(()=>st.SelectMany(o=>st.Where(i=>i.Key.メンバー11.Equals(o.Key.メンバー11))));
        //                            }
        //                        }
        //                    }
        //                } else {
        this.コンパイル実行(() => s.SelectMany(o=>s.Where(i=>o==i&&i+1==o+1)));
        //                }
        ...
        this.コンパイル実行(() => s.SelectMany(o=>new int[3].Where(i=>o==i&&i+1==o+1)));
        //                            : ExtensionEnumerable.Lookup_index;
        this.コンパイル実行(() => s.SelectMany(o=>new int[3].Where((i,index)=>o==i&&i+1==o+1&&index==3)));
        ...
        this.コンパイル実行(() => s.Where(p => true));
        //                    } else {
        this.コンパイル実行(()=>new int[1].Where(p => true));
        //                    }
        //                }
        //            }
        this.コンパイル実行(()=>s.Where(p=>p==0));
        //        }
        this.コンパイル実行(()=>st.SelectMany(o=>s.Let(i=>new{o,i}).i.Where(i=>i==3)));
        this.コンパイル実行(()=>st.SelectMany(o=>s.Where((Func<int,bool>)(i=>i==3))));
        //    }
        this.コンパイル実行(()=>st.Select(o=>new{o}));
        //}
        this.コンパイル実行(()=>st.Let(o=>new{o}));
    }
    //var st=new Set<TestLinqDB.Sets.Key,Sets.Value>{new(new(0)),new(new(1))};
