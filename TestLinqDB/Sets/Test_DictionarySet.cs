...
public class Test_DictionarySet
{
    ...
    public void TryGetValue()
    {
        ...
        }
        {
            ...
            Assert.Equal(new Grouping<int,int>(1) { 2 }, d[1]);
            Assert.Equal(1,d[2].Count);
            Assert.Equal(new Grouping<int,int>(2) { 4 }, d[2]);
            Assert.Equal(1,d[unchecked(1 + int.MaxValue)].Count);
            Assert.Equal(new Grouping<int,int>(unchecked(1 + int.MaxValue)){ 3 }, d[unchecked(1 + int.MaxValue)]);
        }
    }
}