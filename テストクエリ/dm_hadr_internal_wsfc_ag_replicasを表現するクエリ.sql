	SELECT
		...
		--availability_replicas_internal
	FROM (
		SELECT a.*,
		a.group_id internal_group_id,a.replica_id ag_id
		FROM sys.availability_replicas a
	)AS mr
	--sys.dm_hadr_internal_wsfc_ag_replicas 
	FULL JOIN (
		SELECT a.*,
		a.group_id internal_group_id,a.replica_id ag_id,a.replica_id ag_replica_id,a.replica_server_name ag_replica_name
		FROM sys.availability_replicas a
		--JOIN 
	)
	AS wr
