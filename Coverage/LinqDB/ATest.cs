...
#pragma warning restore IDE0009// Member access should be qualified.
    public struct struct_ショートカット検証:IEquatable<struct_ショートカット検証>{
        ...
        public static struct_ショートカット検証 operator^(struct_ショートカット検証 a,struct_ショートカット検証 b)=>
            new(a.Booleanフィールド^b.Booleanフィールド,"("+a.結果+"^"+b.結果+")");
        public static struct_ショートカット検証 operator-(struct_ショートカット検証 a)=>new(-a.Int32フィールド,"-("+a.結果+")");
        public static struct_ショートカット検証 operator+(struct_ショートカット検証 a)=>new(a.Int32フィールド,"+("+a.結果+")");
        public static struct_ショートカット検証 operator+(struct_ショートカット検証 a,struct_ショートカット検証 b)=>
            new(a.Int32フィールド+b.Int32フィールド,"("+a.結果+"+"+b.結果+")");
        public static struct_ショートカット検証 operator~(struct_ショートカット検証 a)=>new(~a.Int32フィールド,"~("+a.結果+")");
    ...
    [NonSerialized]protected readonly Optimizer Optimizer=new(){IsGenerateAssembly = false,Context=typeof(ATest),AssemblyFileName="デバッグ.dll"};
    ...
    protected Action<T> CreateDelegate<T>(Expression<Action<T>> Lambda)=>this.Optimizer.CreateDelegate(Lambda);
    protected Delegate CreateDelegate(LambdaExpression Lambda)=>this.Optimizer.CreateDelegate(Lambda);
    ...
    protected bool IsGenerateAssembly{
        get=>this.Optimizer.IsGenerateAssembly;
        set=>this.Optimizer.IsGenerateAssembly=value;
    }
    /// <summary>
    /// 実行結果が一致するか確認
    /// </summary>
    ...
    /// <typeparam name="TResult"></typeparam>
    /// <param name="Lambda"></param>
    protected void 実行結果が一致するか確認<TResult>(Expression<Func<int,TResult>> Lambda){
        var 標準=Lambda.Compile();
        var Optimizer=this.Optimizer;
        Optimizer.IsInline=false;
        var ラムダ=Optimizer.CreateDelegate(Lambda);
    ...
    protected void 実行結果が一致するか確認<TResult>(Expression<Func<int,int,TResult>> Lambda){
        var 標準=Lambda.Compile();
        var Optimizer=this.Optimizer;
        Optimizer.IsInline=false;
        var ラムダ=Optimizer.CreateDelegate(Lambda);
    ...
    protected void 実行結果が一致するか確認<TResult>(Expression<Func<int,int,int,TResult>> Lambda){
        var 標準=Lambda.Compile();
        var Optimizer=this.Optimizer;
        Optimizer.IsInline=false;
        var ラムダ=Optimizer.CreateDelegate(Lambda);
    ...
    protected void 実行結果が一致するか確認<TResult>(Expression<Func<int,int,int,int,TResult>> Lambda){
        const int N=3;
        var 標準=Lambda.Compile();
        var Optimizer=this.Optimizer;
        Optimizer.IsInline=false;
    ...
    protected void 共通Assert(int expected評価回数,Expression<Func<評価検証,評価検証>> Lambda){
        var 標準=Lambda.Compile();
        var Optimizer=this.Optimizer;
        Optimizer.IsInline=false;
        var ラムダ=Optimizer.CreateDelegate(Lambda);
        ...
        var ループ=Optimizer.CreateDelegate(Lambda);
        {
            var actual=標準(new 評価検証());
            Assert.IsTrue(Comparer.Equals(expected評価回数,actual));
        }
        {
            var actual=ラムダ(new 評価検証());
            Assert.IsTrue(Comparer.Equals(expected評価回数,actual));
        }
        {
            var actual=ループ(new 評価検証());
            Assert.IsTrue(Comparer.Equals(expected評価回数,actual));
        }
    }
    protected void 実行結果が一致するか確認<T>(Expression<Action<T>> Lambda,T input) {
        this.CreateDelegate(Lambda)(input);
    }
    protected TResult 実行結果が一致するか確認<T,TResult>(Expression<Func<T,TResult>> Lambda,T input){
        var Optimizer=this.Optimizer;
        var expected=Lambda.Compile()(input);
        Optimizer.IsInline=false;
        {
            var actual=Optimizer.CreateDelegate(Lambda)(input);
            Assert.IsTrue(Comparer.Equals(expected,actual));
        }
        Optimizer.IsInline=true;
        {
    ...
    protected void 実行結果が一致するか確認(Expression<Action> Lambda,Action 初期化,Action Assert){
        var Optimizer=this.Optimizer;
        Optimizer.IsInline=false;
        var Lambda1=(Expression<Action>)Optimizer.Lambda最適化(Lambda);
        var expected=Lambda1.Compile();
        初期化();
        expected();
        Assert();
        {
            var actual=Optimizer.CreateDelegate(Lambda);
            初期化();
            actual();
            Assert();
        }
    ...
    protected TResult 実行結果が一致するか確認<T0,T1,TResult>(Expression<Func<T0,T1,TResult>> Lambd,T0 a,T1 b){
        dynamic Lambda=Lambd;
        var expected=Lambda.Compile()(a,b);
        var Optimizer=this.Optimizer;
        Optimizer.IsInline=false;
        {
            var actual=Optimizer.CreateDelegate(Lambda)(a,b);
            Assert.IsTrue(Comparer.Equals(expected,actual));
        }
        Optimizer.IsInline=true;
        {
    ...
    protected TResult 実行結果が一致するか確認<TContainer,TResult>(TContainer Container,Expression<Func<TContainer,TResult>> Lambda)
        where TContainer:Container{
        var expected=Lambda.Compile()(Container);
        var Optimizer=this.Optimizer;
        Optimizer.IsInline=false;
        {
            var actual=Optimizer.CreateContainerDelegate(Lambda)(Container);
            Assert.IsTrue(Comparer.Equals(expected,actual));
        }
        Optimizer.IsInline=true;
        {
